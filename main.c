...
//				printf("\n__________________________\n");
		...
		case 3:
			system("Cls");
			printf("\n\t\t\t\t\tYou will be presented with menu.\n\t\t\t\t Enter your choice to perform further actions.");
			getch();
			goto choice1;
			break;
		case 4:
...
{
	int i;
	fp=fopen("files/infoCustomer/idpw.txt","rb");
...
void deductAmt(float amt)
{
	...
	FILE *temp;
	fp=fopen("files/infoCustomer/idpw.txt","rb");
	temp=fopen("files/infoCustomer/temp.txt","wb");
	...
	fclose(fp);
	fclose(temp);
	remove("files/infoCustomer/idpw.txt");
	rename("files/infoCustomer/temp.txt","files/infoCustomer/idpw.txt");
	remove("files/infoCustomer/temp.txt");
}
void sendmoney()
{
	char acNo[8];
...
void addCus()
{
	...
	if(fp!=NULL)
		fclose(fp);
	fp=fopen("files/infoCustomer/idpw.txt","a+b");
	do
	{
		rewind(fp);
		while(fscanf(fp,"%d%s%s%s%s%f",&user.sNo,name_buffer,user.acNo,user.uId,user.uPw,&user.uBal)!=EOF)
